...
const getConfig = async (prefix: string, user: string, ctx: Context) => {
  ...
  if (!results || results.length === 0) {
    config = await initConfig(prefix, user, ctx);
  } else {
    config = results[0];
  }
...
const initConfig = async (prefix: string, user: string, ctx: Context) =>
...
export const apply = (ctx: Context) => {
  ...
  ctx.i18n.define('en-US', {
    ...
    chartFilesAdded: 'Chart files added: {0}',
    ...
    'configSummary.preferences': 'Player Preferences: {0}',
  ...
  const getFileUrl = async (
    file?: { src?: string; fileId?: string; chatId: string; isPrivate: boolean },
    session?: Session<never, never, Context>
  ) => {
    if (file?.src) return file.src;
    if (!file || !session || !file.fileId) return;
  ...
  const validateSocket = async (target: string) => {
    ...
    const room = (await ctx.database.get('pzpAgentRoom', `${prefix}/${user}`)).at(0);
    if (!room) return;
  ...
  const getFiles = (event: Event) => {
    const elements = (event.message.elements.filter((e) => e.type === 'file') || []).concat(
      event.message.quote?.elements.filter((e) => e.type === 'file') || []
    );
    const isPrivate = event.channel.type === 1;
    const chatId = event.channel.id;
    ctx.logger.info('Received files', elements);
  ...
  ctx.on('message', async (session) => {
    ...
    const files = getFiles(session.event);
    if (!files.length) return;
    await prepareSession(session);
    if (pendingRuns[pUser].expectRespack) {
      pendingRuns[pUser].input.respack = files[0];
      pendingRuns[pUser].expectRespack = false;
      await session.send(session.text('respackSet', [files[0].fileName]));
    } else {
      pendingRuns[pUser].input.chartFiles.push(...files);
      await session.send(
        session.text(files.length === 1 ? 'chartFileAdded' : 'chartFilesAdded', [
  ...
  ctx
    ...
    .action(({ session }) => {
      ...
      pendingRuns[pUser].expectRespack = true;
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      if (pendingRuns[pUser].input.chartFiles.length === 0) {
        await session.send(session.text('needChartFiles', [h('at', { id: user })]));
        pendingRuns[pUser].expectRespack = false;
        return;
      }
      const chartFilesText = pendingRuns[pUser].input.chartFiles
        .map((file) => file.fileName)
        .join(', ');
      const respackText = pendingRuns[pUser].input.respack
        ? pendingRuns[pUser].input.respack.fileName
      ...
      const input = {
        chartFiles: await Promise.all(
          pendingRuns[pUser].input.chartFiles.map((file) => getFileUrl(file, session))
        ),
        respack: await getFileUrl(pendingRuns[pUser].input.respack, session)
      };
      const run: Run = {
        input,
        ...config
      };
      const { runId, queueSize, queueTime } = await client.newRun(run, prefix);
      await session.send(
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      if (run.status === 'in_progress') {
        const { status, progress, eta } = await client.getRunProgress(run.id, prefix, user);
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      const id = runs[0].id;
      try {
        await client.cancelRun(id, prefix, user);
  ...
  ctx
    ...
    .action(async ({ session }, page = '1', limit = '3') => {
      ...
      const { total, runs } = await client.getRuns(prefix, user, pageNum, limitNum);
      if (total === 0) {
      ...
      const historyContent = runs
        .map(
          (run, i) =>
            `\n${i + 1 + (pageNum - 1) * limitNum}. [${session.text(`status.${run.status}`)}]${session.text('runId', [run.id])}\n${
  ...
  ctx
    ...
    .action(async ({ session }, property, value) => {
      ...
      await ctx.database.set('pzpAgentConfig', { user: pUser }, { ...data, user: pUser });
    });
  ...
  socket.on('message', async (target: string, status: string, progress: number, eta: number) => {
    ...
    if (!result) return;
    const { prefix, user, runId } = result;
    const pUser = `${prefix}/${user}`;
    await ctx.database.set('pzpAgentRoom', pUser, {
    ...
    if (status === 'initializing') {
      ...
      if (status === 'completed') {
        const run = await client.getRun(runId, prefix, user);
        ...
        if (uploadRetries === 0 || prefix !== 'qq') return;
        await session.sendQueued(session.text('requestSendFile', [h('at', { id: user })]));
            ...
            try {
              ...
              if (error?.name === 'TimeoutError' || error?.constructor?.name === 'TimeoutError') {
                ...
                if (retries >= uploadRetries) {
                  ctx.logger.error('Max retries reached for TimeoutError during upload:', error);
                  await session.send(
                    session.text('requestSendFileFailure', [uploadRetries, h('at', { id: user })])
                  );
                }
              } else {
